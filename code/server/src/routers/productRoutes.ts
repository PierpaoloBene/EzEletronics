...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            "/",
            body("model").exists().isString().isLength({ min: 1 }),
            body("category").exists().isString().isIn(["Smartphone", "Laptop", "Appliance"]),
            body("quantity").exists().isInt({ min: 1 }),
            body("details").optional().isString(),
            body("sellingPrice").exists().isFloat({ gt: 0 }),
            // TODO: test arrival date validation
            body("arrivalDate").optional({ nullable: true }).isISO8601({ strict: true, strictSeparator: true }),
        ...
        this.router.patch(
            "/:model",
            param("model").exists().isLength({ min: 1 }),
            body("quantity").exists().isInt({ gt: 0 }),
            // TODO: test changeDate validation
            body("changeDate").optional({ nullable: true }).isISO8601({ strict: true, strictSeparator: true }),
            this.errorHandler.validateRequest,
            this.authenticator.isAdminOrManager,
            (req: any, res: any, next: any) =>
                this.controller
                    ...
                    .then((quantity: number /**number */) => res.status(200).json({ quantity: quantity }))
                    .catch((err) => next(err))
        )
        ...
        this.router.patch(
            ...
            param("model").exists().isLength({ min: 1 }),
            body("quantity").exists().isInt({ min: 1 }),
            // TODO: test sellingDate validation
            body("sellingDate").optional({ nullable: true }).isISO8601({ strict: true, strictSeparator: true }),
            this.errorHandler.validateRequest,
        ...
        this.router.get(
            "/",
            query("grouping").optional({ nullable: true }).isIn(["category", "model"]),
            Utility.validateQueryCategoryModel,
            this.errorHandler.validateRequest,
            // ABILITARE AUTENTICAZIONE
            this.authenticator.isAdminOrManager,
