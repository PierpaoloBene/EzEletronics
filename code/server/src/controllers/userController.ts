...
class UserController {
    ...
    async getUserByUsername(user: User, username: string) /**:Promise<User> */ {
        if ((user.role !== "Admin") && (user.username !== username)) throw new UnauthorizedUserError
        const ret: any = await this.dao.getUserByUsername(username)
        return ret
     }
    ...
    async deleteUser(user: User, username: string) /**:Promise<Boolean> */ {
        const userToBeDeleted = await this.dao.getUserByUsername(username);
        const isAuthorized = (user.role === "Admin" && (user.username === username || userToBeDeleted.role !== "Admin"))
                      || (user.role !== "Admin" && user.username === username);
        if (!isAuthorized) throw new UnauthorizedUserError
        const ret: any = await this.dao.deleteUser(username)
        return ret
     }
